...
1. **No single signal can ban.** The ban threshold (`raid_guard_score_ban`, default **160**) sits above any one signal's weight (the highest is 50) *and* above any pair of them, so reaching it requires at least three independent things having been true about the same connection at once (`verdictForTier` in `raidguard.go`).
