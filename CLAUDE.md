...
Area-wide moderation for CMs and moderators. `/area mute` silences **everyone in the caller's area except CMs and moderators** — both IC and OOC (`ICOOCMuted`) — and persists by IPID exactly like `/mute` so it survives a quick reconnect. `/area unmute` **reverses it so people can talk again**. The caller, area CMs, CM-permission holders, and moderators are all exempt.

**Scoped to the room (bug fix).** Unlike a real `/mute`, an `/area mute` is meant to silence someone only while they're actually in that area — it used to be indistinguishable from a permanent global mute, so a muted player who simply walked to a different area stayed muted server-wide with no way back, since `/area unmute` can only reach players currently in the CM's area and a CM lacks the `MUTE` permission `/unmute` requires. Each area-scoped mute now records its origin area (`Client.SetAreaMuted`/`AreaMuteOrigin`), and leaving that area — via `ChangeArea` or `forceChangeArea` (e.g. a jail placement) — automatically lifts it (`Client.liftAreaMuteOnDeparture`), clearing both the in-memory state and the persisted DB row. `/area unmute` likewise only reverses a mute whose origin is the caller's own area, so it can never accidentally lift a real `/mute` or one applied by a different room. A full disconnect-and-reconnect elsewhere is not covered by this and still restores the persisted mute, exactly like an ordinary `/mute` would — only an in-session area change auto-lifts it.
