...
**Playtime is hidden the same way, and for a related reason.** Even with the account name gone, the raw playtime figure was still shown unconditionally — and `/playtime top` publicly ranks every registered account by name and exact hours, so someone could match "this anonymous connection has 47h32m" against that leaderboard and deanonymize the target just the same, without ever seeing the account line directly. The `Playtime:` line now shows `(hidden)` under the exact same condition as the account line (someone else's profile, target not authenticated); self-view and an authenticated target are unaffected. Chips and the active cosmetic tag are untouched — those are tracked per-IPID for every connection, guest or not, and are already visible elsewhere (`/gas`, `/players`) regardless of login state, so hiding them on `/profile` alone wouldn't close anything. Implemented in `cmdProfile` (`internal/athena/commands_account.go`).
