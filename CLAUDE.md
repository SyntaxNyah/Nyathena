...
- **Fast character pick (25)** and **character churn (25)** — picking a character in under a second of connecting, or re-rolling faster than a person can read a list of several thousand characters. Measured at **67% of raid char-pickers re-rolling within 1 second vs 0% of baseline**.
...
1. **No single signal can ban.** The ban threshold (`raid_guard_score_ban`, default **140**) sits above any one signal's weight (the highest is 50) *and* above any pair of them, so reaching it requires at least three independent things having been true about the same connection at once (`verdictForTier` in `raidguard.go`).
...
/raidguard status         # settings, live under-attack state, correlation window size,
                           # and every connected client with a nonzero score
