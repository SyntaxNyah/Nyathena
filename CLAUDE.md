...
| `rate_limit_kick_autoban` | `true` | Auto-ban an IPID that racks up repeated message-rate-limit kicks (see "Repeat-Offender Rate-Limit Autoban" below) |
...
### Repeat-Offender Rate-Limit Autoban
A single message-based rate-limit violation (`message_rate_limit`, `ooc_rate_limit` — IC/OOC/music spam, handled by `Client.KickForRateLimit`) has always just resulted in a kick, not a ban, so a bot that reconnects right after being kicked could cycle through kick → reconnect → spam forever with no lasting consequence. When `rate_limit_kick_autoban` is enabled (default: on), an IPID that racks up `rate_limit_kick_autoban_threshold` rate-limit kicks (default: **2**) within a rolling `rate_limit_kick_autoban_window` (default: 600s / 10 minutes) is additionally handed a short automatic ban lasting `rate_limit_kick_autoban_duration` (default: 15 minutes) — long enough to break a spam bot's reconnect loop without being a full moderator-length ban. A kick that arrives after the window has elapsed starts a fresh count instead of adding to an old one, so the autoban targets an active spam spree, not kicks earned far apart over a connection's whole lifetime on the server.

Every call site of `KickForRateLimit` funnels through the same choke point — `pktIC`, `pktChangeChar`, `pktAM` (music/area-change), and both the general and IP-specific OOC rate limiters in `pktOOC` — so the autoban covers IC and OOC alike. In every one of those handlers the rate-limit check runs before any packet parsing or broadcasting, so the message that trips the limit is never broadcast to anyone, kick or autoban alike.
...
- **Fully exempt** — moderators, and any IPID at or above `rate_limit_kick_autoban_min_playtime` (default: 20 hours): never auto-banned by this mechanism no matter how many times it trips the rate limit. Still gets the ordinary kick every time, same as a single violation.
