...
func printCasinoDashboard(client *Client) {
	...
	sb.WriteString("  /crash bet <amount> | /crash cashout   ⚠️  45s cooldown + 5s min hold!\n")
	...
	sb.WriteString(fmt.Sprintf("  %d drinks, ALL with risk & huge variance. Big wins OR big losses!\n", len(barMenu)))
	sb.WriteString("  beer wine whiskey tequila vodka rum gin mojito mead sake champagne\n")
	sb.WriteString("  margarita moonshine absinthe fireball jagerbomb longisland cosmo pina mystery\n")
	sb.WriteString("  poison doubletrouble dragonblood cursedwine goldenelixir roulettebrew\n")
	sb.WriteString("  blackout thundermead devilswhiskey angelwine ghostshot electriclemonade\n")
	sb.WriteString("  voiddrink luckybrew\n")
	sb.WriteString("\n── Other ──\n")
